...
def sweep(f,p,Nv,R,Lv,Ntheta,v,C,dx,n): # sweeping method for the Boltzmann equation
    ...
    # Forward Sweep
    for i in range(1,n):
        # f[i,:,:] = ((dx/v) * Qplus_grid[i,:,:] + f[i-1,:,:])/(1 + (C * p[i] * dx)/v) # Implicit time-stepping
        f[i,:,:] = (dx/v) * Q_grid[i,:,:] + f[i-1,:,:] # Explicit time-stepping

    # Backward Sweep
    for i in range(n-1,0,-1):
        # f[i,:,:] = ((dx/v) * Qplus_grid[i,:,:] + f[i-1,:,:])/(1 + (C * p[i] * dx)/v) # Inplicit time-stepping
        f[i,:,:] = (dx/v) * Q_grid[i,:,:] + f[i-1,:,:] # Explicit time-stepping
...
def weno1_boltzmann(f,p,u1,u2,T,pL,pR,uL,uR,TL,TR,v,C,n,dx,Nv,R,Lv,Ntheta,V1,V2,v_grid,x_grid,save_me,save_freq):
    ...
    f0 = f
    f1 = np.copy(f0)
    f1 = sweep(f1,p,Nv,R,Lv,Ntheta,v,C,dx,n) # update phase space probability density
    ...
    f2 = np.copy(f1)
    f2 = sweep(f2,p,Nv,R,Lv,Ntheta,v,C,dx,n) # update phase space probability density
